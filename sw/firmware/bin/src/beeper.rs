...
fn setup(p: &Peripherals) {
    // Enable TIM1 clock.
    p.device.RCC.apb2enr.modify(|_, w| w.tim1en().set_bit());

    // Set prescaler, the counter clock frequency (CK_CNT) is equal to f(CK_PSC) / (PSC[15:0] + 1).
    p.device.TIM1.psc.modify(|_, w| unsafe { w.bits(0b0) });
    ...
    p.device.TIM1.arr.write(|w| unsafe { w.bits(timer_period) });

    // Set repetition counter.
    p.device.TIM1.rcr.write(|w| unsafe { w.bits(0b0) });
    ...
    p.device.TIM1.cr2.modify(|_, w| {
        // Set output Idle state 1 (OC1 output and OC1N output).
        w.ois1().set_bit().ois1n().clear_bit()
    });

    // Enable counter.
    p.device.TIM1.cr1.modify(|_, w| w.cen().set_bit());
}

fn teardown(p: &Peripherals) {
    // Disable counter.
    p.device.TIM1.cr1.modify(|_, w| w.cen().clear_bit());

    // Disable TIM1 clock.
    p.device.RCC.apb2enr.modify(|_, w| w.tim1en().clear_bit());
}
