...
impl PWMBeeperHardware for SystemHardwareImpl {
    fn enable_pwm(&self) {
        // Enable TIM1 clock.
        self.p.RCC.apb2enr.modify(|_, w| w.tim1en().enabled());
        ...
        self.p.TIM1.arr.write(|w| w.arr().bits(timer_period));
        ...
        self.p.TIM1.ccr3.write(|w| w.ccr().bits(channel_one_pulse));
        ...
        // Enable counter.
        self.p.TIM1.cr1.modify(|_, w| w.cen().enabled());

        self.p.TIM1.bdtr.modify(|_, w| w.moe().enabled());
    }

    fn disable_pwm(&self) {
        self.p.TIM1.bdtr.modify(|_, w| w.moe().disabled_idle());

        // Disable counter.
        self.p.TIM1.cr1.modify(|_, w| w.cen().disabled());

        // Disable TIM1 clock.
        self.p.RCC.apb2enr.modify(|_, w| w.tim1en().disabled());
    }
    ...
    fn pulse(&self, note_frequency: u32) {
        ...
        self.p.TIM1.arr.write(|w| w.arr().bits(frequency));
    }
}
