...
if [ "$1" != "" ]; then
  BUILD_COMMAND="cargo build --target $TARGET -p $PROJECT --example $1"
  BINARY_PATH="debug/examples/$1"
else
...
if ! $BUILD_COMMAND; then
    echo -e "\n\e[31mFailed to build required binary\e[0m\n"
    exit 1
else
    echo -e "\n\e[32mSuccessfully built binary\e[0m\n"
fi
